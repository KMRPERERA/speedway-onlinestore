...
const Main = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserCounts = async () => {
      try {
        const response = await fetch('https://onlinestorebackend20250502182239.azurewebsites.net/api/UserAuthentication/getusercounts');
        if (!response.ok) {
        ...
        const countsObj = {};
        data.forEach(item => {
          countsObj[item.entity] = item.count;
        });
        
        setUserCounts(countsObj);
      } catch (error) {
        ...
        <section className="statistics-section">
          ...
          <div className="stats-container">
            <div className="stat-box">
              ...
              <div className="stat-number">{userCounts.Customers}</div>
              <div className="stat-label">Customers</div>
            </div>
            
            <div className="stat-box">
              <div className="stat-icon">👨‍💼</div>
              <div className="stat-number">{userCounts.Suppliers}</div>
